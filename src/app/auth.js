...
import { clearSessionCookie, sessionCookie } from "../utils/session.js";
...
export async function login(request, env) {
  ...
  if (await authenticateUser(env.DB, username, password)) {
    return redirect("/chat", { cookie: sessionCookie(username) });
  }
...
export function logout() {
  return redirect("/auth", { cookie: clearSessionCookie() });
}
